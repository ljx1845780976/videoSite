...
//    }
    ...
    public String addVideo( VideoDto videoDto) throws IOException {
        //第一步：处理并写入视频上传的视频，并获得视频路径
          MultipartFile videoFile=videoDto.getVideoFile();
            if(!videoFile.isEmpty()){
                ...
                File videoDir = new File(userDir.getAbsolutePath() + "\\video");
                File imgDir = new File(userDir.getAbsolutePath() + "\\img");
                if (!userDir.exists()) {
                    userDir.mkdir();
                    //为用户创建视频目录和图片目录
                    videoDir.mkdir();
     ...
     //第二步：通过ffmpeg获取视频时长及写入视频封面图片和获取路径
                ...
                String imgUrl = VideoUtils.getImgUrl(videoUrl, imgDir.getAbsolutePath());
                String videoTime = VideoUtils.getVideoTime(videoUrl);
